...
def load_shrek_3_srt() -> List[srt.Subtitle]:
    subs: List[srt.Subtitle]

    srt_path = str(pathlib.Path(__file__).parent.parent) + "/data/Shrek_3.srt"
...
def load_only_shrek_from_shrek_3_srt() -> List[srt.Subtitle]:
    subtitles = load_shrek_3_srt()
...
subtitles = load_only_shrek_from_shrek_3_srt()

import pprint
# pprint.pprint(subtitles)

print(f"Getting {len(subtitles)} distinct training sound bytes for Shrek 3")
